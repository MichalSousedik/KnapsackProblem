...
bool asc (std::pair<int,int> i,std::pair<int,int> j) { return (i.first<j.first); }
bool desc (std::pair<int,int> i,std::pair<int,int> j) { return (i.first>j.first); }
...
int main (int argc, char* argv[]) {
	...
	printf ("%d %d %d",I+j, n, M);
        ...
        for (i=0; i<n; i++)
            vector.push_back(std::make_pair(costs[i], weights[i]));
	...
	for (i=0; i<n; i++) printf (" %d %d", vector[i].second, vector[i].first);
	printf ("\n");
